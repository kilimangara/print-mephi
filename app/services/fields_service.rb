module FieldsService
  ...
  def pre_build_order_fields
    ...
    CustomField.where(destiny: CustomField::ORDER_D, active: true).each do |c_f|
      ...
      session[:order_fields].push(custom_field)
    end
  end

  def last_not_fulfilled
    @to_fulfill = session[:order_fields].reject { |item| item[:fulfilled] }.first
  end
  ...
  def resolve_by_type!(field_type, message)
    ...
    when CustomField::FILE_TYPE
      ...
      index = session[:order_fields].index { |item| item[:field_type] == field_type && !item[:fulfilled] }
      return nil unless index
      session[:order_fields].at(index).merge!(value: file_obj.fetch('file_id'), fulfilled: true)
    when CustomField::TEXT_TYPE
      text = message.fetch('text', nil)
      return nil unless text
      index = session[:order_fields].index { |item| item[:field_type] == field_type && !item[:fulfilled] }
      return nil unless index
      session[:order_fields].at(index).merge!(value: text, fulfilled: true)
    end
  end
end
